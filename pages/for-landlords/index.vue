<template>
  <div>
    <div class="mt-12 container">
      ...
      <h1 class="text-4xl xl:w-[80%] xl:text-7xl">{{$t('landlords.intro')}}</h1>
    ...
    <div class="mt-24 container">
      <h3 data-aos="fade-right" class="text-4xl lg:w-1/2">{{$t('landlords.list_title')}}</h3>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-2 gap-x-40 gap-y-12 mt-12">
        <div class="flex flex-col gap-8" data-aos="fade-up" data-aos-delay="200">
          <p>01.</p>
          <h3 class="text-2xl font-bold">{{$t('landlords.list1_title')}}</h3>
        ...
        <div class="flex flex-col gap-8" data-aos="fade-up" data-aos-delay="400">
          <p>02.</p>
          <h3 class="text-2xl font-bold">{{$t('landlords.list2_title')}}</h3>
          <p class="text-lg text-brand-secondary">{{$t('landlords.list2_p')}}</p>
        </div>
        <div class="flex flex-col gap-8" data-aos="fade-up" data-aos-delay="200">
          <p>03.</p>
          <h3 class="text-2xl font-bold">{{$t('landlords.list3_title')}}</h3>
          <p class="text-lg text-brand-secondary">{{$t('landlords.list3_p')}}</p>
        </div>
        <div class="flex flex-col gap-8" data-aos="fade-up" data-aos-delay="400">
          <p>04.</p>
          <h3 class="text-2xl font-bold">{{$t('landlords.list4_title')}}</h3>
    ...
    <section class="mt-24 overflow-hidden relative">
      <div class="flex flex-nowrap gap-8 first-of-type:mt-0 mt-4" :class="`selector${i}`" v-for="i in 3" :key="i">
        <nuxt-img
          v-for="j in 4"
          :key="j"
          :src="`/images/landlords-grid/${j + 4*(i-1)}.jpg`"
          format="webp"
    ...
    <section class="mt-24 py-12 container">
      <div class="flex justify-center items-center flex-col gap-4 text-black">
        <h3 class="text-3xl text-center xl:text-[36px] xl:leading[43px] xl:max-w-[471px] tracking-[0.04em] font-bold" data-aos="fade-up">{{$t('contact_us_3')}}</h3>
...
export default {
  head() {
    ...
    return {
      link: [...i18nHead.link],
      title: this.$t('seo.landlords.title'),
      meta: [
        {
          hid: "description",
          name: "description",
          content: this.$t('seo.landlords.description'),
        },
        {
          hid: "og:title",
          name: "og:title",
          content: this.$t('seo.landlords.title'),
        },
        {
          hid: "og:description",
          name: "og:description",
